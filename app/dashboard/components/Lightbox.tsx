  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelectedImage(null)}>
      <div className="relative max-w-5xl max-h-screen">
        <img src={selectedImage} className="max-w-full max-h-[90vh]" alt="Enlarged" />
        <button onClick={() => setSelectedImage(null)} className="absolute -top-10 right-0 text-white font-bold text-xl ">✕ Close</button>
      </div>
    </div>
  );
}